...
const BAR_COLORS = ["oklch(0.72 0.17 162)", "oklch(0.68 0.13 195)", "oklch(0.78 0.16 85)", "oklch(0.65 0.2 305)"];
              ...
              <tbody>
                {jobs.map((j) => (
                  <tr key={j.id} className="border-b border-border/40 hover:bg-accent/30 transition">
                    ...
                    <td className="p-3 text-right font-mono text-emerald-400 tabular-nums">{j.auto}</td>
                    <td className="p-3 text-right font-mono text-amber-400 tabular-nums">{j.human}</td>
                    <td className="p-3 text-right">
                      <span className={cn("font-mono tabular-nums", j.autoRate >= 0.5 ? "text-emerald-400" : "text-amber-400")}>
                        {Math.round(j.autoRate * 100)}%
                      </span>
                    </td>
                    <td className="p-3 text-right font-mono tabular-nums">{j.avgConfidence.toFixed(2)}</td>
                    <td className="p-3 text-right">
                      <span className={cn("font-mono tabular-nums", j.kappaChapter.tone === "good" && "text-emerald-400", j.kappaChapter.tone === "warn" && "text-amber-400", j.kappaChapter.tone === "bad" && "text-rose-400")}>
                        {j.kappaChapter.value.toFixed(3)}
                      </span>
                    </td>
                    <td className="p-3 text-right">
                      <div className="flex items-center justify-end gap-1.5 text-xs">
                        <span className="text-emerald-400 font-mono">{j.honeypotPass}</span>
      ...
      <div className="grid lg:grid-cols-2 gap-5">
        <ChartCard title="Auto vs Human rate" desc="Routing split per job" icon={GitBranch}>
          <BarChart data={autoRateData} margin={{ top: 8, right: 8, bottom: 24, left: -10 }}>
            ...
            <YAxis tick={{ fill: "oklch(0.68 0.02 250)", fontSize: 11 }} unit="%" />
        ...
        <ChartCard title="Average confidence" desc="Mean confidence per job" icon={Target}>
          <BarChart data={confData} margin={{ top: 8, right: 8, bottom: 24, left: -10 }}>
            ...
            <Bar dataKey="Avg Confidence" radius={[4, 4, 0, 0]}>
              {confData.map((d, i) => (
                <Cell key={i} fill={d["Avg Confidence"] >= 0.85 ? "oklch(0.72 0.17 162)" : d["Avg Confidence"] >= 0.6 ? "oklch(0.78 0.16 85)" : "oklch(0.66 0.22 25)"} />
              ))}
            </Bar>
          </BarChart>
        </ChartCard>
        ...
        <ChartCard title="Inter-annotator agreement" desc="Fleiss' κ per job" icon={TrendingUp}>
          <BarChart data={kappaData} margin={{ top: 8, right: 8, bottom: 24, left: -10 }}>
            ...
            <Bar dataKey="Fleiss κ" radius={[4, 4, 0, 0]}>
              {kappaData.map((d, i) => (
                <Cell key={i} fill={d["Fleiss κ"] > 0.8 ? "oklch(0.72 0.17 162)" : d["Fleiss κ"] >= 0.6 ? "oklch(0.78 0.16 85)" : "oklch(0.66 0.22 25)"} />
              ))}
            </Bar>
          </BarChart>
        </ChartCard>
        ...
        <ChartCard title="Honeypot results" desc="Pass vs fail per job" icon={ShieldCheck}>
          <BarChart data={honeypotData} margin={{ top: 8, right: 8, bottom: 24, left: -10 }}>
            ...
            <Tooltip contentStyle={{ background: "oklch(0.205 0.018 250)", border: "1px solid oklch(1 0 0 / 10%)", borderRadius: 8 }} cursor={{ fill: "oklch(1 0 0 / 5%)" }} />
            <Legend wrapperStyle={{ fontSize: 12 }} />
            <Bar dataKey="Pass" stackId="a" fill="oklch(0.72 0.17 162)" radius={[0, 0, 0, 4]} />
            <Bar dataKey="Fail" stackId="a" fill="oklch(0.66 0.22 25)" radius={[4, 4, 0, 0]} />
          </BarChart>
        </ChartCard>
      </div>
    </div>
